...
- C# runtime/editor code 작성과 수정 (씬/프리팹을 대신 만드는 `[MenuItem]` Builder/Validator Editor 스크립트 포함)
- 문서 작성과 정합성 수정
- 테스트 코드 작성
- 코드/문서 검색, 빌드, 검증
- 설계 tradeoff 설명
...
```text
씬에 있는 오브젝트 위치를 대신 옮겨줘.
```

응답:

```text
직접 옮기지 않습니다. Hierarchy에서 해당 오브젝트를 선택하고 Inspector의 Position 값을 요청하신 좌표로 설정해 주세요. 단순 위치 변경이라 스크립트는 필요 없습니다.
```
...
- 직접 수정하지 않습니다.
- 단순 배치·값 변경(오브젝트 이동, 필드 값 하나 수정 등)은 정확한 메뉴 경로와 값을 프로즈로 지시합니다. 이 경우 스크립트 없이 완료 처리합니다.
- 다음 중 하나에 해당하면 `PrefabUtility.SaveAsPrefabAsset` 또는 `EditorSceneManager.SaveScene`을 호출하는 `[MenuItem]` Builder/Installer Editor 스크립트를 작성해 사람이 한 번 실행하게 합니다: 참조/컴포넌트 연결이 여러 개 얽힘, 나중에 같은 구조를 다시 만들 가능성이 있음, 잘못 만들면 바로 눈에 띄지 않는 실패(참조 끊김, 이벤트 미연결 등)가 남음.
- 이런 Builder는 파일 상단에 `// ONE-OFF: <대상 경로> 생성용. 결과 확인 후 팀 합의하에 삭제 가능.` 주석을 남깁니다. 실행·확인이 끝났다고 AI가 임의로 삭제하지 않습니다 — 삭제 여부와 시점은 사람이 팀과 조율해서 정합니다.
- 위 기준으로 복잡하다고 판단한 경우에 한해, 상태를 스캔해 문제를 로그로 알려주는 `[MenuItem]` Validator를 추가로 작성합니다. Validator는 재검증용이므로 삭제하지 않고 유지합니다.
...
- 바뀐 판단 기준
- 검증 결과
- 사람이 Unity에서 해야 할 일 — 씬/프리팹 작업이 있었다면 실행할 정확한 메뉴 경로와, 실행 후 눈으로 확인할 항목(Game View, Inspector 연결, 애니메이션/타이밍)
- 남은 위험
